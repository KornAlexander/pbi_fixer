...
# LinkedIn Post â€” Structure Chart Fix (IBCS Columnâ†’Bar)
...
- **Time series** â†’ vertical columns (left to right)
- **Structural comparisons** â†’ horizontal bars (top to bottom)
...
1. Scans every chart's category axis
2. Checks if the data type is DateTime or matches time patterns (Year, Month, Quarter, Date...)
3. Non-time categories on column charts â†’ converts to bar chart
4. Time categories on bar charts â†’ converts to column chart
5. Preserves stacked/clustered formatting

Additionally, bar charts get sorted descending by the measure â€” so the longest bar is always at the top. That's IBCS convention for structural comparisons.
